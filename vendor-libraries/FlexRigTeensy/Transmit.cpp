...
Transmit::Transmit() {
	...
	upd_dax=true; 
	upd_pitch=true; 
    upd_speed = false; // Critical fix: do not raise an initial speed event before radio reports a value
	upd_iambic=true;
	...
	// Mark "unknown" until the radio sends speed/wpm.
    speed = -1;
...
void Transmit::setValue(String msg){
	...
	if (i>0) {
		...
		if (var.equals(F("wpm"))) { 
			set_speed(msg.substring(i+1).toInt()); 
			return; 
		} 

		if (var.equals(F("speed"))) { 
			set_speed(msg.substring(i+1).toInt()); 
			return; 
		} 
