...
Radio::Radio() {
  ...
  // --- deterministic defaults for state values ---
  slices               = 0;
  panadapters          = 0;
  lineout_gain         = 0;
  lineout_mute         = 0;
  ...
  // --- mark all as "needs update" so initial fireEvents() can emit them once ---
  upd_slices             = true;
  ...
  // --- make sure all handlers start as null ---
  do_slices_event             = NULL;
  do_panadapters_event        = NULL;
  do_lineout_gain_event       = NULL;
  do_lineout_mute_event       = NULL;
  do_headphone_gain_event     = NULL;
  do_headphone_mute_event     = NULL;
  do_remote_on_enabled_event  = NULL;
  do_pll_done_event           = NULL;
  do_freq_error_ppb_event     = NULL;
  do_cal_freq_event           = NULL;
  do_tnf_enabled_event        = NULL;
  do_snap_tune_enabled_event  = NULL;
  do_nickname_event           = NULL;
  do_callsign_event           = NULL;
  do_binaural_rx_event        = NULL;
  do_disconnected_event       = NULL;
}
...
void Radio::updateStatus(String msg) {
  ...
  // Tokenize by spaces until newline/end. Be careful not to skip characters.
  int oldi = 0;
  int i = msg.indexOf(' ', oldi);
  while (i > 0) {
    setValue(msg.substring(oldi, i));
    oldi = i + 1;
    i = msg.indexOf(' ', oldi);          // <-- (fix) do NOT add +1 here
  }

  // get the last element (until newline or end of string)
  int nl = msg.indexOf('\n', oldi);
  if (nl < 0) nl = msg.length();
  if (nl > oldi) {
    setValue(msg.substring(oldi, nl));
  }
...
void Radio::setValue(String msg) {
  ...
  if (i > 0) {
    ...
    if (var.equals(F("cal_freq"))) { 
      // value expressed in MHz; library uses Hz (float to int conversion preserved)
      set_cal_freq((int)(msg.substring(i + 1).toFloat() * 1000000.0f));
      return; 
    }
...
void Radio::fire_disconnected_event() {
  // (fix) actually invoke the callback if present
  if (do_disconnected_event != NULL) {
    do_disconnected_event();
  }
}
...
void Radio::updateObject(int objectId, int value) {
  switch (objectId) {
    ...
    default:
      Serial.print(F("Radio::updateObject - Unhandled id:"));
